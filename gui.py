...
from calssifier_2 import classify_zeta_with_cnn
import cv2
import numpy as np
...
class Application(tk.Frame):
    ...
    def createOptionFrames(self, images, names):
        if not hasattr(self, "optionLabel"):
            self.optionLabel = {}
        if not hasattr(self, "optionCheck"):
            self.optionCheck = {}
        if not hasattr(self, "optionVar"):
            self.optionVar = {}
        ...
        for i, name in enumerate(names):
            ...
            self.optionVar[name] = tk.BooleanVar(value=True)
    ...
    def sendToArduino(self, piece_type):
        if piece_type == "Unknown":
            return
        print(self.optionVar[piece_type].get())
        if not self.optionVar[piece_type].get():
            return
...
def main():
    ...
    appRunCamera = Application(
        master=root, src=0  #"./Vid_Piezas/Arandelas/Arandelas_Buenas.mp4"
    )
